...
class Camera1 extends CameraController implements Camera.PreviewCallback, Camera.ErrorCallback {
    ...
    private void setupContinuousFocusMove(){
        if (mContinuousFocusModeIsStarted){
            mContinuousFocusModeIsStarted = false;
            mCameraCallbacks.dispatchOnContinuousFocusMove(false);
        }
        if(isCameraAvailable()){
            setContinuousFocusMoveCallback(new ContinuousFocusMoveCallback() {
                @Override
                public void onContinuousFocusMove(boolean start) {
                    if( start != mContinuousFocusModeIsStarted ) {  // filter out repeated calls with same start value
                        mContinuousFocusModeIsStarted = start;
                        mCameraCallbacks.dispatchOnContinuousFocusMove(start);
                    }
                }
            });
        }
    ...
    private void setContinuousFocusMoveCallback(final ContinuousFocusMoveCallback cb) {
